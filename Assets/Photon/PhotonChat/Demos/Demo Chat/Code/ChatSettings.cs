...
namespace Photon.Chat.DemoChat {
	...
	public class ChatSettings : ScriptableObject {
...
#if PUN_2_OR_NEWER
				if (instance == null) {
					instance = CreateInstance<ChatSettings>();
				}

				instance.AppId = Pun.PhotonNetwork.PhotonServerSettings.AppSettings.AppIdChat;
				return instance;
		...
		private static ChatSettings Create() {
			ChatSettings settings = (ChatSettings)CreateInstance("ChatSettings");
#if UNITY_EDITOR
